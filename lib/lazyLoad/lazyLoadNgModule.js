...
export function applyNgModule(transition, ng2Module, parentInjector, lazyLoadState) {
    ...
    if (isFuture) {
        ...
        if (!replacementState || replacementState === originalState) {
            throw new Error("The Future State named '".concat(originalName, "' lazy loaded an NgModule. ") +
                "The lazy loaded NgModule must have a state named '".concat(replacementName, "' ") +
                "which replaces the (placeholder) '".concat(originalName, "' Future State. ") +
                "Add a '".concat(replacementName, "' state to the lazy loaded NgModule ") +
